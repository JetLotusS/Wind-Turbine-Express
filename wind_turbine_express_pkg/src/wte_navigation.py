...
class WTENavigationNode(Node):
    ...
    def __init__(self):
        ...
        carte_pixel = self.precharger_carte_distance("dist_transform.jpg")
        taille_carte_dist_transform = carte_pixel.shape
        G = self.recup_graphe("graph_rocher.txt")
        ...
        l_eolienne = [(-190.07841337181242, 0.127446451792), (-105.59533937321473, -41.79734152154377),
                    (139.4011694854088, 85.02789988405654)]

        l_eolienne = self.wind_turbines_coordinates

        l_graphe_eolienne = self.genere_graphe_eolienne(l_eolienne)

        pos = (0,0) # ON SUPPOSE QUIL APPARAIT AU MILIEU OUI C FAUX JE C
        d, ordre_visite = self.plus_court_chemin(l_eolienne, pos)
        # print(ordre_visite)
        ...
        liste_eolienne_chemin = []

        #Liste des rochers deja atteint
        rocher_deja_atteint = []
        p_eolien_deja_atteint = []
    ...
    def nav_point_callback(self):
        Nextpoint = [0,0] #Doit pouvoir recup le next point jsp oû
        nav_msg = Thruster() 
        nav_msg.x = Nextpoint[0]
        nav_msg.y = Nextpoint[1]
    ...
    def get_distance_caillou(self,pos):
        """
            Renvoie la vitesse recommandé (entre 0 et 1) en fonction de l'image des distance eulerienne
        """
        x_carte = self.taille_carte_dist_transform[0] * (pos[0] + 300) / 600
        y_carte = self.taille_carte_dist_transform[1] * (pos[1] + 300) / 600
    ...
    def point_atteint(self,p_atteint):
        ...
        for i_e in range(len(self.l_eolienne)):
            p_e = self.l_eolienne[i_e]
    ...
    def next_point(self,pos):
        """
        Renvoie la prochaine position à atteindre depuis la position pos
        ...
        if len(self.liste_chemin_en_cours) != 0:
            ...
            next_pos,i_eolienne = self.prochain_points_etape1(self.l_eolienne, self.eolienne_vu, self.G, pos, (-9999999, -999999))
            return next_pos,i_eolienne

    def prochain_points_etape1(self,l_eolienne, eolienne_vu, G_caillou, pos, vect_vitesse):
        """
        ...
        d_caillou = self.get_distance_caillou(pos)
        ...
        caillou_a_portee = []
        for i in range(n):
            if self.dist(pos, V[i]) < r and i not in self.rocher_deja_atteint: #On ignore les rochers déjà atteint
        ...
        eoliennes_a_portee = []
        for i in range(len(l_eolienne)):
            if self.dist(pos,l_eolienne[i]) < r and i != i_eolienne:
                eoliennes_a_portee.append(i)



        RANGE = 30
        vitesse_actuelle = RANGE * (255 - self.get_distance_caillou(pos)) / 255
        ...
        points_qui_coupent = []
        s_direction = (pos, self.add_vect(pos, vect_eolienne))
        ...
        for i in range(len(points_qui_coupent)):
            ...
            if d_proche > self.dist(pos, p_rocher):
                d_proche = self.dist(pos, p_rocher)
